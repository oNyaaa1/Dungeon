...
local function DrawWorldTip(tip)
    if IsValid(tip.ent) then tip.pos = tip.ent:GetPos() end
    ...
    verts[3] = {
        x = pos.x - tipOffset / 2 + 2,
        y = pos.y - tipOffset / 2 + 2
    }
...
function GM:PaintWorldTips()
    if WorldTip and WorldTip.dietime > SysTime() then DrawWorldTip(WorldTip) end
end